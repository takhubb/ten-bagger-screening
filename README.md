...
- 時価総額が 100 億円以下
- 自己資本比率が 20% 以上
- 直近四半期売上成長率（前年同期比）が 10% 以上
- 直近四半期営業利益成長率（前年同期比）が 10% 以上

日付未指定時は「今日」を基準に、その日時点で使える最新営業日で実行します。
`--date` や `--from-date` / `--to-date` で明示指定した非営業日はスキップし、出力しません。
...
日付未指定で実行した場合は、その日時点で使える最新営業日を基準日にします。
`--date` や `--from-date` / `--to-date` で指定した日付が土日祝などの非営業日だった場合、その日はスキップされ、ターミナル表示も CSV 出力も行いません。

例:

- `2026-04-11` を指定
- その日が非営業日
- `2026-04-11` 分の結果は出力されない

実行時には、標準エラー出力にスキップした旨が表示されます。
...
- スクリーニング条件では `PBR <= 1.0` の銘柄だけを採用します
- 分母が取得できない場合や 0 以下の場合は空欄になります
- `当四半期売上成長率(前年同期比)` は、四半期単体売上高ベースで算出します
- `当四半期営業利益成長率(前年同期比)` は、四半期単体営業利益ベースで算出します
- 前年同期の営業利益が取得できない場合や 0 以下の場合、営業利益成長率は空欄になります
...
- `--volume-history-buffer-days`: 出来高履歴取得に使う暦日バッファ
- `--weekly-lookback-weeks`: 52週高値判定に使う週数
- `--weekly-history-buffer-days`: 52週高値判定に使う価格履歴の暦日バッファ
- `--financial-history-days`: 四半期業績の算出に使う財務データの遡及日数
- `--backtrack-days`: 日付未指定時に最新営業日に寄せる最大遡り日数
